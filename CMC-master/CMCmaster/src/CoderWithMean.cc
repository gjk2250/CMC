...
CoderWithMean::CoderWithMean(size_t buffer_sz, size_t used_memory, uint32_t max_len, uint8_t min, uint8_t max): max_len(max_len), buffer_size(buffer_sz), used_m(used_memory), min_value(min), max_value(max) {
}

CoderWithMean::CoderWithMean(size_t buffer_sz, size_t used_memory, uint32_t max_len): max_len(max_len),
																					  buffer_size(buffer_sz),
																					  used_m(used_memory) {
}

CoderWithMean::~CoderWithMean() = default;

void CoderWithMean::get_max_and_min_value(const char* input_file) {
	try {
...
int CoderWithMean::compress(const char* input_file, const char* output_file) {
	...
	try {
		...
		while (!in_file.feof()) {
			...
			out1 = new char[100000000];
			RangeCoder rc_mean;
			rc_mean.output(out1 + 8);
			rc_mean.StartEncode();
			...
			for (uint32_t i = 0; i < row_count; ++i) {
				...
				if (mean[i] < (NUM_SYMBOL - 13))
					mean[i] = NUM_SYMBOL - 13;
				else if (mean[i] < (NUM_SYMBOL - 11))
					mean[i] = NUM_SYMBOL - 11;
				else if (mean[i] < (NUM_SYMBOL - 9))
					mean[i] = NUM_SYMBOL - 9;
				else if (mean[i] < (NUM_SYMBOL - 7))
					mean[i] = NUM_SYMBOL - 7;
				else if (mean[i] < (NUM_SYMBOL - 5))
					mean[i] = NUM_SYMBOL - 5;

				model_qual[pre].encodeSymbol(&rc_mean, mean[i]);
			}

			rc_mean.FinishEncode();
			uint32_t sz0 = rc_mean.size_out();
			...
			out1[3] = (sz0 >> 24) & 0xff;
			/* save lines number*/
			out1[4] = (row_count >> 0) & 0xff;
			out1[5] = (row_count >> 8) & 0xff;
			...
			for (uint32_t j = 0; j < col_max; ++j) {
				for (uint32_t k = 0; k < row_count; ++k) {
					...
					int F = mean[i];
					int q_model_idx = 0, g_model_idx = 0;
					...
					if (j == 0) {
						...
						if (F < 30) {
							...
							q_model_idx = A * 5 * 5 * 2 * 2 * a + F * 2 * 2 * a + C * 2 * a + M;
							g_model_idx = G1;
...
int CoderWithMean::decompress(const char* compress_file, const char* fasta_file, const char* output_file) {
	...
	try {
		...
		while (!in_file.feof()) {
			...
			rc_mean.StartDecode();
			for (uint32_t i = 0; i < len_buf[1]; ++i)
				mean[i] = model_qual[pre].decodeSymbol(&rc_mean);
...
void CoderWithMean::set_min_value(uint8_t min) {
	min_value = min;
}
void CoderWithMean::set_max_value(uint8_t max) {
	max_value = max;
}
uint8_t CoderWithMean::get_min_value() const {
	return min_value;
}
uint8_t CoderWithMean::get_max_value() const {
	return max_value;
}
